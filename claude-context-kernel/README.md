...
| `CK_PARK` / `CK_PARK_MAX` / `CK_PARK_KEEP` / `CK_PARK_TTL` | `1` / 512 KB / `40` / 24h | output parking of elided ephemeral results; `recall.py` for targeted recovery |
...
| `~/.context-kernel-park.json` | zlib'd **full originals of elided tool outputs** (Bash/MCP/WebFetch — needed for targeted recall; may contain anything a command printed) | ≤ 512 KB/entry, last 40, TTL 24h | `CK_PARK=0` |
