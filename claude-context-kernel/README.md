...
- **189 tests**: 185 Python contract tests (pure stdlib, ~15s) + 4 Pi bridge
...
```bash
npm test                                # 185 Python + 4 Pi bridge tests
# Claude-only baseline:
cd claude-context-kernel
python3 -m unittest discover -s tests    # 185 tests, ~15s, stdlib only
```
