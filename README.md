...
a small representative of $[C]_Q$. One honesty note the notation owes you:
there is no such thing as "the" projector either. The valid projectors for a
task form a **family** $\Pi_Q = \{\pi_1, \pi_2, \ldots\}$ — many different
slices are equivalent under $A_Q$ — and the shipped pipeline is a *chosen
member*, canonical by engineering decision (deterministic, zero-cost,
reproducible), not by theorem. Since 1.33.0 the choice inside the family is
itself principled where it can be: among the depth-ladder configs, $T_2$ picks
$\arg\min_i |\pi_i(C)|$ *subject to sufficiency* (§2.2). The agent never works
on the repository; it works on the task state. Concretely, $\pi_Q$ is factored
...
statusline commands and prints one **slim** line by default: model · project ·
session savings · historical share, plus the `⚠ canary` alarm when it fires
(an alarm always shows; counters don't):

```
Fable 5 · kernel · ck ⚡ -5.1k · tot -64%
```

`CK_STATUSLINE_VERBOSE=1` brings back the full diagnostic line — session
figure related to the context you *would have had* without compression
(current context from the tracker + tokens saved), historical total with
absolute tokens, pending A/B samples, cumulative fault tokens:

```
Fable 5 · kernel · ck ⚡ -5.1k sessione (-5% su ctx ~98.2k) · -369k totale (-64%) · A/B: 3 in attesa · ↩12.0k fault
```

Colors are standard 16-color ANSI so they follow your terminal theme —
savings in green, canary alarm in red, pending A/B in yellow, the `ck ⚡`
brand in yellow (`CK_STATUSLINE_BRAND` = red/green/yellow/blue/magenta/
cyan/none), icon+text always primary (`CK_STATUSLINE_COLOR=0` for plain
text).
